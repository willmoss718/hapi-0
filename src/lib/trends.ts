...
export async function loadAllTrends(): Promise<TrendRecord[]> {
  ...
  const state: TrendRecord[] = stateCsv
    .map((row: Record<string, string>) => {
      ...
      return {
        ...
        title: row['Policy Name/Number'] || row['Policy Name'] || 'State Policy',
        tags: {
          keyword: splitTags(row['Keyword Tags']),
          implications: splitTags(row['Healthcare Implications']),
  ...
  const fed: TrendRecord[] = fedCsv
    .map((row: Record<string, string>) => {
      ...
      return {
        ...
        title: row['Policy Name/Number'] || row['Policy Name'] || 'Federal Policy',
        tags: {
          keyword: splitTags(row['Keyword Tags']),
          implications: splitTags(row['Healthcare Implications']),
          impact: splitTags(row['Impact Level']),
  ...
  return [...state, ...fed, ...sec, ...vol, ...intl].sort((a, b) => a.date.localeCompare(b.date));
}
